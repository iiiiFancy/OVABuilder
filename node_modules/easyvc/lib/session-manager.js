...
class SessionManager {
	...
	async ensureSession(host) {
		...
		try {
			await svc.vimPort.currentTime(svc.serviceInstance)
			return svc
		} catch (e) {
			console.log(`vSphere API session timeout. Idle till now: ${(interval/1000/60)|0} minutes. Refreshing...`)
			try {
				await svc.vimPort.logout(svc.serviceContent.sessionManager)
			} catch (e) {
			}

			await svc.vimPort.login(svc.serviceContent.sessionManager, session.user, session.password)
