...
class BaseMob {
	constructor(service, mor) {
		...
		if (!sessionMgr)
			sessionMgr = require('./session-manager.js')
		let host = sessionMgr.findSessionHost(service)
		if (!host)
			throw 'BaseMob: host not found for service session'
		let easyvc = require('./index.js')()
		easyvc.host = host
		
		Object.defineProperty(this, '_easyvc', {value: easyvc})

		this.mor = mor		
	}

	_serviceDirect() {
		return this._easyvc._serviceDirect()
	}

	_service() {
		return this._easyvc._service()
	}
	...
	async get(name) {
		...
		let ret = await svc.vimPort.retrievePropertiesEx(svc.serviceContent.propertyCollector, [
